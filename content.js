...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'REQUEST_SCREENSHOT_INFO') {
    handleScreenshotInfo().then(sendResponse);
    return true; // async
...
async function handleScreenshotInfo() {
  const video = document.querySelector('video');
  const titleEl = document.querySelector('h1.ytd-watch-metadata');
  
  const title = titleEl ? titleEl.innerText : 'Unknown Video';
  const timestamp = video ? formatTime(video.currentTime) : '0:00';
  const urlParams = new URLSearchParams(window.location.search);
  const videoId = urlParams.get('v');

  // Launch quick caption popup and waiting for user input
  const caption = await showCaptionPopup();
    ...
    let timeoutId = setTimeout(() => {
      ...
      if (input.value.trim() === '') {
        finish('');
         ...
         // Auto-submit whatever they typed after idle timeout of 3s maybe? 
         // For a better UX, if they typed but paused, we shouldn't steal it too aggressively.
         // Let's dismiss empty or submit whatever is there.
